...
function modal(html, extraClass = '') {
  stopCamera();
  const isWizard = extraClass.includes('wizard-modal');
  const dlg = document.getElementById('modal');
  const wizardPanel = document.getElementById('wizard-panel');
  const wizardContent = document.getElementById('wizard-content');

  // IMPORTANT iOS: l'assistant guidé n'utilise plus <dialog>.
  // Les dialogues natifs peuvent rester dans la "top layer" et bloquer la barre du bas.
  if (isWizard) {
    if (dlg.open) dlg.close();
    wizardContent.className = `modal-card ${extraClass}`.trim();
...
function setupNavigation() {
  ...
  // Délégation au niveau document : plus robuste sur Safari/PWA que des handlers isolés.
  document.addEventListener('click', event => {
    const nav = event.target.closest?.('.bottom-nav .nav-item');
    if (nav) {
      navigate(nav.dataset.view || nav.getAttribute('href')?.slice(1));
      return;
    }
  ...
  // Secours tactile iOS : active dès la fin du toucher si Safari ne produit pas de click normal.
  document.querySelectorAll('.bottom-nav .nav-item').forEach(item => {
    item.addEventListener('touchend', () => navigate(item.dataset.view), { passive: true });
  });

  window.addEventListener('hashchange', () => {
    const id = location.hash.slice(1);
    if (document.getElementById(id)?.classList.contains('view')) showView(id, false);
  });

  const brand = document.getElementById('brand-home');
  if (brand) brand.onclick = () => navigate('home-view');
  const settings = document.getElementById('open-settings');
  if (settings) settings.onclick = () => { try { closeModal(); } catch {} showView('settings-view'); };

  const initialHash = location.hash.slice(1);
  if (document.getElementById(initialHash)?.classList.contains('view')) showView(initialHash, false);
}
...
function groupFamily(group) {
  ...
  if (g.includes('pector')) return 'Pectoraux';
  if (g.includes('dos')) return 'Dos';
  if (g.includes('épaule')) return 'Épaules';
  if (g.includes('biceps')) return 'Biceps';
  if (g.includes('triceps')) return 'Triceps';
  if (g.includes('jamb') || g.includes('quad')) return 'Quadriceps';
  if (g.includes('ischio')) return 'Ischios';
  if (g.includes('mollet')) return 'Mollets';
  if (g.includes('fess')) return 'Fessiers';
  if (g.includes('core')) return 'Core';
  if (g.includes('mobil')) return 'Mobilité';
  if (g.includes('cardio') || g.includes('endurance') || g.includes('puissance') || g.includes('vitesse')) return 'Cardio';
  return 'Autre';
}
function exerciseType(ex) { const f = groupFamily(ex.group); return f === 'Cardio' ? 'cardio' : f === 'Mobilité' ? 'mobilite' : 'fitness'; }

const FITNESS_FOCUS_GROUPS = [
...
function focusLabel(selected) {
  if (!selected) return 'Sélectionne une zone';
  return selected;
}
function focusFamily(selected = '') {
  const s = String(selected).toLowerCase();
  if (s.includes('pector')) return 'Pectoraux';
  if (s.includes('grand dorsal') || s.includes('trap') || s === 'dos') return 'Dos';
  if (s.includes('delto') || s.includes('épaule')) return 'Épaules';
  if (s.includes('biceps') || s.includes('triceps') || s.includes('avant')) return 'Bras';
  if (s.includes('quad') || s.includes('ischio') || s.includes('mollet') || s === 'jambes') return 'Jambes';
  if (s.includes('fess')) return 'Fessiers';
  if (s.includes('abdo') || s.includes('oblique') || s === 'core') return 'Abdominaux';
  if (s.includes('full')) return 'Full body';
  if (s.includes('mobil')) return 'Mobilité';
  return selected || 'Full body';
}
function focusSide(selected = '') {
  const s = String(selected).toLowerCase();
  if (['dos', 'grand dorsal', 'trapèzes', 'fessiers', 'ischios', 'mollets'].some(v => s.includes(v))) return 'back';
  if (s.includes('triceps')) return 'back';
  return 'front';
}
function focusTransform(selected = '') {
  const family = focusFamily(selected);
  const map = {
    'Bras': 'translate(-34,-10) scale(1.28)',
    'Pectoraux': 'translate(-8,-6) scale(1.18)',
    'Dos': 'translate(-8,-6) scale(1.18)',
    'Épaules': 'translate(-16,-14) scale(1.22)',
    'Jambes': 'translate(-10,-46) scale(1.28)',
    'Fessiers': 'translate(-12,-44) scale(1.24)',
    'Abdominaux': 'translate(-4,-18) scale(1.24)',
    'Full body': 'translate(0,0) scale(1)',
    'Mobilité': 'translate(0,0) scale(1)'
  };
  return map[family] || 'translate(0,0) scale(1)';
}
function isMuscleActive(selected, names) {
  if (!selected || selected === 'Full body') return selected === 'Full body';
...
function bodyMapSVG(selected = '') {
  ...
  const active = (...names) => isMuscleActive(selected, names) || (selected === 'Full body' && !names.includes('none')) ? 'active-muscle' : '';
  const front = `
  <g transform="${transform}">
    <circle cx="100" cy="30" r="20"/>
    <path class="body-base" d="M72 54Q100 38 128 54L144 114Q136 132 127 149L124 250H105L100 178L95 250H76L73 149Q64 132 56 114Z"/>
    <path class="body-limb" d="M72 58 52 116 64 124 80 88Z"/>
    <path class="body-limb" d="M128 58 148 116 136 124 120 88Z"/>
    <path class="body-limb" d="M80 250 72 310 90 310 98 250Z"/>
    <path class="body-limb" d="M120 250 110 310 128 310 120 250Z"/>

    <path class="muscle ${active('Pectoraux')}" d="M78 68Q100 58 122 68L114 102Q100 112 86 102Z"/>
    <path class="muscle ${active('Épaules','Deltoïdes')}" d="M58 60Q68 50 77 55L73 84 57 82ZM123 55Q132 50 142 60L143 82 127 84Z"/>
    <path class="muscle ${active('Biceps','Bras')}" d="M54 92Q66 90 72 100L69 130Q61 137 52 132Z"/>
    <path class="muscle ${active('Biceps','Bras')}" d="M128 100Q134 90 146 92L148 132Q139 137 131 130Z"/>
    <path class="muscle ${active('Avant-bras','Bras')}" d="M52 132Q60 130 69 136L66 164Q58 166 50 160Z"/>
    <path class="muscle ${active('Avant-bras','Bras')}" d="M131 136Q140 130 148 132L150 160Q142 166 134 164Z"/>
    <path class="muscle ${active('Core','Abdos')}" d="M86 106H114L116 154H84Z"/>
    <path class="muscle ${active('Obliques','Core')}" d="M76 112 84 116 82 154 72 144ZM124 112 116 116 118 154 128 144Z"/>
    <path class="muscle ${active('Fessiers')}" d="M80 154Q100 146 120 154L116 182Q100 188 84 182Z"/>
    <path class="muscle ${active('Quadriceps','Jambes')}" d="M82 182H98L95 248H76ZM102 182H118L124 248H105Z"/>
    <path class="muscle ${active('Mollets','Jambes')}" d="M78 248H92L88 300H72ZM108 248H122L126 300H110Z"/>
  </g>`;
  const back = `
  <g transform="${transform}">
    <circle cx="100" cy="30" r="20"/>
    <path class="body-base" d="M72 54Q100 42 128 54L145 118Q134 137 126 151L124 250H105L100 178L95 250H76L74 151Q66 137 55 118Z"/>
    <path class="body-limb" d="M72 60 52 116 64 124 80 90Z"/>
    <path class="body-limb" d="M128 60 148 116 136 124 120 90Z"/>
    <path class="body-limb" d="M80 250 72 310 90 310 98 250Z"/>
    <path class="body-limb" d="M120 250 110 310 128 310 120 250Z"/>

    <path class="muscle ${active('Dos','Grand dorsal')}" d="M78 70Q100 58 122 70L126 118Q100 130 74 118Z"/>
    <path class="muscle ${active('Trapèzes','Dos')}" d="M82 56Q100 48 118 56L120 76Q100 86 80 76Z"/>
    <path class="muscle ${active('Épaules','Deltoïdes')}" d="M58 60Q68 52 77 56L74 84 58 82ZM123 56Q132 52 142 60L142 82 126 84Z"/>
    <path class="muscle ${active('Triceps','Bras')}" d="M54 92Q66 92 72 102L68 132Q61 139 52 135Z"/>
    <path class="muscle ${active('Triceps','Bras')}" d="M128 102Q134 92 146 92L148 135Q139 139 132 132Z"/>
    <path class="muscle ${active('Avant-bras','Bras')}" d="M52 134Q60 132 68 137L66 165Q58 167 50 161Z"/>
    <path class="muscle ${active('Avant-bras','Bras')}" d="M132 137Q140 132 148 134L150 161Q142 167 134 165Z"/>
    <path class="muscle ${active('Fessiers')}" d="M78 152Q100 144 122 152L118 182Q100 190 82 182Z"/>
    <path class="muscle ${active('Ischios','Jambes')}" d="M82 182H98L94 246H78ZM102 182H118L122 246H106Z"/>
    <path class="muscle ${active('Mollets','Jambes')}" d="M78 246H92L88 300H72ZM108 246H122L126 300H110Z"/>
  </g>`;
...
function multiGroupBodyMapSVG(selectedGroups = []) {
  ...
  const front = `<svg class="body-map multi-body-map front-map anatomical-map" viewBox="0 0 260 430" aria-label="Mannequin full body vue de face">
    <circle cx="130" cy="34" r="24"/>
    <path class="body-base" d="M102 58Q130 46 158 58L170 84Q182 116 186 145L176 182Q169 196 165 216L161 310H143L136 220H124L117 310H99L95 216Q91 196 84 182L74 145Q78 116 90 84Z"/>
    <path class="body-limb" d="M94 88Q76 116 64 156Q59 174 65 194L76 188Q78 166 86 147Q94 126 108 106Z"/>
    <path class="body-limb" d="M166 88Q184 116 196 156Q201 174 195 194L184 188Q182 166 174 147Q166 126 152 106Z"/>
    <path class="body-limb" d="M74 194Q80 212 86 248Q89 269 86 294L100 294Q106 267 109 234Q111 209 111 188Z"/>
    <path class="body-limb" d="M186 194Q180 212 174 248Q171 269 174 294L160 294Q154 267 151 234Q149 209 149 188Z"/>
    <path class="body-limb" d="M106 310Q101 348 94 404L114 404Q121 367 126 328L128 310Z"/>
    <path class="body-limb" d="M154 310Q159 348 166 404L146 404Q139 367 134 328L132 310Z"/>

    <path class="muscle ${on('Épaules')}" d="M84 78Q96 63 110 68Q108 91 104 108Q92 109 81 101Q80 89 84 78Z"/>
    <path class="muscle ${on('Épaules')}" d="M176 78Q164 63 150 68Q152 91 156 108Q168 109 179 101Q180 89 176 78Z"/>
    <path class="muscle ${on('Pectoraux')}" d="M104 84Q117 77 130 79Q143 77 156 84L148 123Q139 134 130 138Q121 134 112 123Z"/>
    <path class="muscle ${on('Bras')}" d="M72 114Q84 112 93 121Q97 143 93 165Q84 174 72 170Q66 150 72 114Z"/>
    <path class="muscle ${on('Bras')}" d="M188 114Q176 112 167 121Q163 143 167 165Q176 174 188 170Q194 150 188 114Z"/>
    <path class="muscle ${on('Bras')}" d="M76 184Q88 180 97 189Q99 210 95 234Q86 242 76 238Q72 212 76 184Z"/>
    <path class="muscle ${on('Bras')}" d="M184 184Q172 180 163 189Q161 210 165 234Q174 242 184 238Q188 212 184 184Z"/>
    <path class="muscle ${on('Abdominaux')}" d="M116 138H144L148 219H112Z"/>
    <path class="muscle ${on('Abdominaux')}" d="M102 143Q110 144 114 149L111 217Q103 218 97 210Z"/>
    <path class="muscle ${on('Abdominaux')}" d="M158 143Q150 144 146 149L149 217Q157 218 163 210Z"/>
    <path class="muscle ${on('Fessiers')}" d="M110 219Q130 210 150 219L145 247Q130 256 115 247Z"/>
    <path class="muscle ${on('Jambes')}" d="M108 247Q120 244 128 249L124 334Q116 344 106 339Q102 304 108 247Z"/>
    <path class="muscle ${on('Jambes')}" d="M152 247Q140 244 132 249L136 334Q144 344 154 339Q158 304 152 247Z"/>
    <path class="muscle ${on('Jambes')}" d="M102 338Q111 333 120 338L116 402H96Q95 367 102 338Z"/>
    <path class="muscle ${on('Jambes')}" d="M158 338Q149 333 140 338L144 402H164Q165 367 158 338Z"/>
  </svg>`;

  const back = `<svg class="body-map multi-body-map back-map anatomical-map" viewBox="0 0 260 430" aria-label="Mannequin full body vue de dos">
    <circle cx="130" cy="34" r="24"/>
    <path class="body-base" d="M102 58Q130 48 158 58L170 86Q181 118 184 147L176 183Q168 198 164 216L160 310H143L136 220H124L117 310H100L96 216Q92 198 84 183L76 147Q79 118 90 86Z"/>
    <path class="body-limb" d="M94 90Q76 118 65 158Q61 176 67 194L78 188Q80 168 88 149Q96 126 108 108Z"/>
    <path class="body-limb" d="M166 90Q184 118 195 158Q199 176 193 194L182 188Q180 168 172 149Q164 126 152 108Z"/>
    <path class="body-limb" d="M76 194Q82 214 87 249Q89 269 86 294L100 294Q106 267 109 234Q111 210 111 188Z"/>
    <path class="body-limb" d="M184 194Q178 214 173 249Q171 269 174 294L160 294Q154 267 151 234Q149 210 149 188Z"/>
    <path class="body-limb" d="M106 310Q101 348 94 404L114 404Q121 367 126 328L128 310Z"/>
    <path class="body-limb" d="M154 310Q159 348 166 404L146 404Q139 367 134 328L132 310Z"/>

    <path class="muscle ${on('Épaules')}" d="M84 80Q96 66 110 70Q107 92 103 108Q91 110 81 101Q80 90 84 80Z"/>
    <path class="muscle ${on('Épaules')}" d="M176 80Q164 66 150 70Q153 92 157 108Q169 110 179 101Q180 90 176 80Z"/>
    <path class="muscle ${on('Dos')}" d="M108 80Q120 72 130 74Q140 72 152 80L162 142Q147 161 130 167Q113 161 98 142Z"/>
    <path class="muscle ${on('Dos')}" d="M92 88Q101 82 109 87L105 149Q95 156 84 146Q84 116 92 88Z"/>
    <path class="muscle ${on('Dos')}" d="M168 88Q159 82 151 87L155 149Q165 156 176 146Q176 116 168 88Z"/>
    <path class="muscle ${on('Bras')}" d="M72 114Q84 111 93 120Q97 142 93 165Q84 174 72 170Q66 150 72 114Z"/>
    <path class="muscle ${on('Bras')}" d="M188 114Q176 111 167 120Q163 142 167 165Q176 174 188 170Q194 150 188 114Z"/>
    <path class="muscle ${on('Bras')}" d="M76 184Q88 180 97 189Q99 210 95 234Q86 242 76 238Q72 212 76 184Z"/>
    <path class="muscle ${on('Bras')}" d="M184 184Q172 180 163 189Q161 210 165 234Q174 242 184 238Q188 212 184 184Z"/>
    <path class="muscle ${on('Fessiers')}" d="M110 219Q130 210 150 219L146 248Q130 259 114 248Z"/>
    <path class="muscle ${on('Jambes')}" d="M108 248Q118 243 126 249L122 332Q114 341 104 336Q101 303 108 248Z"/>
    <path class="muscle ${on('Jambes')}" d="M152 248Q142 243 134 249L138 332Q146 341 156 336Q159 303 152 248Z"/>
    <path class="muscle ${on('Jambes')}" d="M102 336Q111 332 120 338L116 402H96Q95 369 102 336Z"/>
    <path class="muscle ${on('Jambes')}" d="M158 336Q149 332 140 338L144 402H164Q165 369 158 336Z"/>
  </svg>`;
...
function dominantExerciseTarget(ex) {
  ...
  if (group.includes('quad')) return 'Quadriceps';
  if (group.includes('ischio')) return 'Ischios';
  if (group.includes('mollet')) return 'Mollets';
  if (group.includes('fess')) return 'Fessiers';
  if (group.includes('épaule')) return 'Deltoïdes';
  if (group.includes('dos')) return 'Grand dorsal';
  if (group.includes('pector')) return 'Pectoraux';
  if (group.includes('core')) return 'Abdos';
  if (group.includes('cardio') || group.includes('endurance') || group.includes('puissance') || group.includes('vitesse')) return 'Full body';
  if (group.includes('mobil')) return 'Full body';
  return groupFamily(ex.group);
}
function exerciseMatchesFocus(ex, focusMain, focusDetail) {
  const main = (focusMain || '').toLowerCase();
  const group = ex.group.toLowerCase();
  const name = ex.name.toLowerCase();
  if (!main || main === 'full body') return true;
  if (main === 'pectoraux') return group.includes('pector') || name.includes('pompes') || name.includes('développé') || name.includes('écartés') || name.includes('dips');
  if (main === 'dos') return group.includes('dos') || name.includes('tractions') || name.includes('rowing') || name.includes('tirage');
  if (main === 'épaules') return group.includes('épaule') || name.includes('face pull') || name.includes('développé militaire') || name.includes('élévations') || name.includes('oiseau');
  if (main === 'bras') return group.includes('biceps') || group.includes('triceps') || name.includes('curl') || name.includes('triceps') || name.includes('barre au front');
  if (main === 'jambes') return group.includes('jamb') || group.includes('quad') || group.includes('ischio') || group.includes('mollet') || name.includes('squat') || name.includes('fentes') || name.includes('presse');
  if (main === 'fessiers') return group.includes('fess') || name.includes('hip thrust');
  if (main === 'abdominaux') return group.includes('core') || name.includes('planche') || name.includes('crunch') || name.includes('relevés de jambes');
  if (main === 'mobilité') return group.includes('mobil');
  return true;
}
...
function renderWorkoutWizard() {
  ...
  if (step === 3) {
    if (workoutWizard.type === 'fitness') {
      ...
      document.getElementById('wizard-next').onclick = () => { workoutWizard.step = 4; renderWorkoutWizard(); };
      return;
    }
    const focuses = workoutWizard.type === 'cardio' ? ['Endurance', 'HIIT', 'Vitesse', 'Puissance'] : ['Mobilité', 'Abdominaux', 'Full body'];
...
// Service worker temporairement désactivé pendant les tests iPhone pour éviter les anciennes versions en cache.
